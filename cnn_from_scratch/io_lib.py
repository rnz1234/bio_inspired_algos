...
class Reader(object):
    ...
    def _read_file(self, file_path, inc_header=False, is_test_set=False, is_valid_set=False):
            ...
            for row in csv_reader:
                ...
                if not is_test_set:
                    if DEBUG_LIMIT_LINES_READ:
                        if is_valid_set:
                            if line >= DEBUG_NUM_OF_LINES_READ_LIMIT/4:
                                break
                        else:
                ...
                if line % 1000 == 999:
                    print("Reader : line number = " + str(line+1))
    ...
    def read_validation_set(self):
        ...
        self.validation_set = self._read_file(file_path=self.validation_set_path, is_valid_set=True)
        if DEBUG_READER:
...
class Writer(object):
    ...
    # write int data
    def write(self, data, special=False):
        if special:
